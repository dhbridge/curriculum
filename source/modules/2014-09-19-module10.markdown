...
We can get the title, description, and subject headings by looking for those "keys" within each item in our "json_data" list. 
    ...
    def get_text(json_data):
        for each in json_data:
            # Get the title
            found_titles = each['sourceResource']['title']

            # Get the Description
            found_descriptions = each['sourceResource']['description']

            # Get the Subject Headings
            found_subjects = each['sourceResource']['subject']
...
There are two more steps we need to get the text ready to save. The first step is to get all of the data out of lists and make sure that it is in the form of a string.
    ...
    def get_text(json_data):
        for each in json_data:
            ...
            try:
                found_titles = each['sourceResource']['title']
                if isinstance(found_titles, basestring):
                    title = str(found_titles)
                else:
                    for each in found_titles:
                        title = str(each)
            except:
                title = ' '
            ...
            try:
                found_descriptions = each['sourceResource']['description']
                if isinstance(found_descriptions, basestring):
                    description = str(found_descriptions)
                else:
                    for line in found_descriptions:
            ...
            try:
                found_subjects = each['sourceResource']['subject']
                if isinstance(found_subjects, basestring):
                    subject = str(found_subjects)
                else:
                    for each in found_subjects:
                        subject = []
